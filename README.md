...
| `Q1` | NPN 三极管 | S8050 / 2N3904 (TO-92) | 1 | 左声道蜂鸣器驱动放大 |
| `R1, R2`| 直插五色环电阻 | 10kΩ, ±1% 1/4W | 2 | ADC 1.65V 直流分压偏置 |
| `R3, R4, R5`| 直插五色环电阻 | 1kΩ, ±1% 1/4W | 3 | 输出短路保护与三极管基极电阻 |
...
### 3.5 后级输出与蜂鸣器网络 (新增 NPN 三极管驱动)
为了解决 DAC 直接驱动蜂鸣器电流不足（声音极小）的问题，左声道加入标准 NPN 三极管作为电流放大：
```
[ PCM5102A LOUT ] ---> C2 (10uF) 正极 
                       C2 (负极) ---> R5 (1kΩ) ---> NPN 三极管 基极(B)
                                                    NPN 发射极(E) ---> GND
                                                    NPN 集电极(C) ---> 蜂鸣器 负极 (-)
                                                                       蜂鸣器 正极 (+) ---> 5V 或 3V3
```
* **左声道 (监听)**：`LOUT` (Pin 15) $\to$ `C2` (10µF) 正极；`C2` 负极 $\to$ `R5` (1kΩ) $\to$ `Q1` (NPN) 基极 `B`。
* **三极管 (Q1)**：发射极 `E` 接 `GND`；集电极 `C` 接 蜂鸣器负极。蜂鸣器正极接 `5V`。
