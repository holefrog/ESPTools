//2.42TFT,2.80TFT,240*320
//GMT024-08 模块 2.4寸 8PIN SPI TFT FOR AT89S52/STC12
//AT89S52驱动
//ic:ST7789控制器,
...
sbit cs        =P3^4;//片选
sbit reset     =P3^2;//复位
sbit rs        =P3^3;//地址/数据切换
sbit sda       =P3^1;// SDI  MOSI
sbit scl       =P3^0;//时钟
//SDO引脚可以不连接
...
//************************************************
	...
	write_command(0xC2);		//VDV and VRH Command Enable
	write_data(0x01);

	write_command(0xC3);			//VRH Set
	write_data(0x0F);		//4.3+( vcom+vcom offset+vdv)

	write_command(0xC4);			//VDV Set
	write_data(0x20);				//0v

	write_command(0xC6);				//Frame Rate Control in Normal Mode
	write_data(0X01);			//111Hz

	write_command(0xd0);				//Power Control 1
	write_data(0xa4);
	write_data(0xa1);

	write_command(0xE8);				//Power Control 1
	write_data(0x03);

	write_command(0xE9);				//Equalize time control
	write_data(0x09);
	write_data(0x09);
	write_data(0x08);
...
void  RamAdressSet()
{
   ...
   LCD_DataWrite(0x01,0x3f);

   write_command(0x2B);

   LCD_DataWrite(0x00,0x00);
   LCD_DataWrite(0x00,0xef);
